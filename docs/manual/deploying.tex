...
probably some messages printed to the screen.  
...
\subsection{Some helpful environment variables}

\shell{procer takes only two parameters to start: the profiles directory and a PID files.
It normally writes its log files (\file{run.log} and \file{error.log} in the profiles directory.
If that is not where you want them you can set the environment variables \shell{PROCER_RUN_LOG} and. 
\shell{PROCER_ERROR_LOG} to change the names of the files, or if you provide an (absolute) pathname to these
environments, also their location. 

In its turn \shell{procer} sets the following environment variables for each profile it starts:
\begin{description}
\item [PROCER_PIDFILE] this is set to the content of the \file{pid_file} of the profile, so it is easier to
keep this in sync with your \file{run} script.
\item [PROCER_ACTION] this is set to the name of the current profile, so it is easier to rueuse \file{run} scripts
